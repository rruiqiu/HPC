...
namespace swiftware::hpp {
    ...
    void gemvT1(int m, int n, const float *A, const float *x, float *y, ScheduleParams Sp) {
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; j += tileSize) {
                ...
                if(i==23){
                    std::cout<<std::setprecision(17) << std::fixed <<  "gemvT1" <<y[i] <<std::endl;
                }
            }
        }
    }
    ...
    void gemvVec(int m, int n, const float *A, const float *x, float *y, ScheduleParams Sp) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if(i==23){
                //stop at 23:
                std::cout<< "gemvVec" << std::setprecision(17) << std::fixed << y[i] <<std::endl;
                std::cout << "Max precision for float: " << std::numeric_limits<float>::digits10 << " decimal digits\n";
            }
        }
    }
