...
namespace swiftware::hpp {
  ...
  TEST(SPMMTest, TestAll) {
    ...
    // std::vector<int> m_values = {1,2,32,64,128,512,100,150,250,300,400,33,1};
    // std::vector<int> n_values = {1,2,32,64,128,512,100,100,200,350,400,31,22};
    // std::vector<int> k_values = {1,2,32,64,128,512,100,100,200,300,450,33,3};
    ...
    for(int srPercentage:srPercentages){
      for(size_t i=0; i< m_values.size();i++){
        ...
        swiftware::hpp::spmmCSR(m, n, k, ACSR->rowPtr.data(), ACSR->colIdx.data(), ACSR->data.data(),  B->data.data(), C->data.data(), swiftware::hpp::ScheduleParams(32, 32));

        swiftware::hpp::spmmCSROptimized(m, n, k, ACSR->rowPtr.data(), ACSR->colIdx.data(), ACSR->data.data(),  B->data.data(), Test_1->data.data(), swiftware::hpp::ScheduleParams(32, -1));
        ...
        std::cout<< srPercentage <<"  Complete\n";
        delete A;
        delete B;
        delete C;
        delete Test_1;
